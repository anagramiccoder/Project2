...
NOTE: Assumption is that if the command exists, it should contain the right arguments, for example
read <path>
write <path> <string>
empty <path>
...
--the write command
after 1 or 6 second delay, will append letter per letter of the given string to the path indicated. in case file does not exist, it will create a new file in the given path and write the string letter per letter with 25 ms interval each. The string may contain alphanumeric and non-alphanumeric characters but should not be space( although the code still accepts it, there should be no space inputs).
